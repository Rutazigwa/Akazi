...
def test_a_recent_successful_run_with_an_empty_queue_is_ok(session, ran_minutes_ago):
    ran_minutes_ago(2)
    assert messaging_status(session)["state"] == "ok"


def test_a_dispatcher_that_stopped_is_reported_as_stalled(session, ran_minutes_ago):
    ran_minutes_ago(STALE_AFTER_MINUTES + 5)
    status = messaging_status(session)
    assert status["state"] == "stalled"
    assert "last ran" in status["reason"]


def test_one_slow_tick_is_not_an_outage(session, ran_minutes_ago):
    """A threshold that cries wolf gets turned off, which is worse."""
    ran_minutes_ago(STALE_AFTER_MINUTES - 1)
    assert messaging_status(session)["state"] == "ok"
...
def test_messages_stuck_past_their_time_are_reported(session, make_candidate, ran_minutes_ago):
...
def test_a_sent_message_is_not_a_backlog(session, make_candidate, ran_minutes_ago):
    ran_minutes_ago(1)
    queue_message(session, make_candidate, OVERDUE_AFTER_MINUTES + 10,
                  status="sent")
    assert messaging_status(session)["state"] == "ok"
...
def test_a_stalled_dispatcher_outranks_a_backlog(session, make_candidate, ran_minutes_ago):
...
def test_the_dashboard_says_so_when_nothing_is_going_out(web, session, ran_minutes_ago):
    ran_minutes_ago(STALE_AFTER_MINUTES + 5)
    page = web.get("/ui/")
    assert "Messages are not going out" in page.text


def test_the_dashboard_stays_quiet_when_messages_are_moving(web, session, ran_minutes_ago):
    ran_minutes_ago(2)
    assert "Messages are not going out" not in web.get("/ui/").text


def test_old_runs_are_pruned(session, ran_minutes_ago):
    ran_minutes_ago(60 * 24 * 45)
    ran_minutes_ago(5)
